...
function _mRepairSymbolicUrlAnchors(a){a=String(a||"");a=a.replace(/\(\s*(__QTMURL\d+__)\s+(\[[^\]\r\n]{1,16}\])/g,"$2 ($1)");a=a.replace(/\(\s*((?:https?:\/\/|www\.)[^()\s]+)\s+(\[[^\]\r\n]{1,16}\])/gi,"$2 ($1)");_mTrace("RESPONSE_SYMBOLIC_ANCHOR_REPAIRED text="+_mTraceCut(a));return a}
...
function serviceTranslateResponseFreeTranslations(a,b,c,d){var e,f,g;if(_mForceFirstError)return _mFail("FreeTranslations","Testfehler",c,d,"serviceTranslateRequestWebTran");_mTrace("RESPONSE_RAW text="+_mTraceCut(b));g=_paParse(b);_mTrace("RESPONSE_PARSED text="+_mTraceCut(g));g=_mRepairSymbolicUrlAnchors(g);f=_mRestoreUrls(g);e=_mFormat(a,f);_mTrace("RESPONSE_FORMATTED text="+_mTraceCut(e));if(_mValid(e))return new ResponseData(_mMark("FreeTranslations",e),c,d);return _mFail("FreeTranslations","keine verwertbare Antwort",c,d,"serviceTranslateRequestWebTran")}
